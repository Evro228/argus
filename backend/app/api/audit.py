        ...
        for pattern in SECRET_PATTERNS:
            ...
            for m in matches:
                ...
                if len(secret_snippet) > 8:
                    masked = secret_snippet[:4] + "*" * (len(secret_snippet) - 8) + secret_snippet[-4:]
                else:
                    masked = "****"
...
def scan_directory_path(req: ScanPathRequest):
    ...
    if not os.path.exists(target_path):
        return {"success": False, "error": f"Путь '{target_path}' не существует на диске."}

    all_findings = []
    files_scanned = 0
    
    # Ignore big/irrelevant directories
    ignored_dirs = {".git", "node_modules", ".venv", "venv", "__pycache__", "build", "dist", ".cache"}
    ...
    if os.path.isfile(target_path):
        ...
        for root, dirs, files in os.walk(target_path):
            ...
            for file in files:
                ...
                if file.endswith(('.py', '.js', '.ts', '.jsx', '.tsx', '.json', '.env', '.yaml', '.yml', '.xml', '.txt', '.sh', '.conf', '.md', '.sql')):
