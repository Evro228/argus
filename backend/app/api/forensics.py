...
async def extract_image_exif(file: UploadFile = File(...)):
    ...
    try:
        ...
        return {"success": False, "error": f"Не удалось открыть изображение: {str(e)}"}
        ...
        if gps_info:
            try:
                ...
                if gps_lat and gps_lon:
                    lat_deg = convert_to_degrees(eval(gps_lat) if isinstance(gps_lat, str) and "[" in gps_lat else gps_lat)
                    if gps_lat_ref == "S":
                        lat_deg = -lat_deg
                    lon_deg = convert_to_degrees(eval(gps_lon) if isinstance(gps_lon, str) and "[" in gps_lon else gps_lon)
                    if gps_lon_ref == "W":
                        lon_deg = -lon_deg
            except Exception:
                pass
    ...
    return {
        ...
        "coordinates": {
            "latitude": lat_deg,
            "longitude": lon_deg,
            "osm_url": f"https://www.openstreetmap.org/?mlat={lat_deg}&mlon={lon_deg}#map=16/{lat_deg}/{lon_deg}" if lat_deg else None,
            "google_maps_url": f"https://www.google.com/maps?q={lat_deg},{lon_deg}" if lat_deg else None
        },
        "camera": {
