...
Скачайте официальный образ установщика [ARGUS-1.0.0-arm64.dmg](https://github.com/Evro228/argus/releases/latest/download/ARGUS-1.0.0-arm64.dmg) и перетащите `ARGUS.app` в папку «Программы» (`/Applications`).

> **⚠️ Важно для macOS (Gatekeeper):**
> ARGUS распространяется как open-source проект без платной подписи Apple Developer (\$99/год).
> При скачивании через Safari/Chrome macOS добавляет флаг карантина и выводит:
> *«Приложение повреждено, и его не удается открыть»*.
>
> **Приложение НЕ повреждено.** Выполните в Терминале **перед первым запуском**:
> ```bash
> # Вариант 1: Снять карантин с DMG-файла (до открытия DMG)
> xattr -d com.apple.quarantine ~/Downloads/ARGUS-1.0.0-arm64.dmg
>
> # Вариант 2: Снять карантин с уже установленного приложения
> xattr -cr /Applications/ARGUS.app
> ```
> *Альтернатива: откройте «Системные настройки» → «Конфиденциальность и безопасность» → нажмите **«Все равно открыть»** (Open Anyway).*
