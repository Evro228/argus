...
4. **NASA FIRMS (Спутниковые термоточки):** Мониторинг тепловых аномалий и пожаров высокого разрешения (сенсоры VIIRS / MODIS).
5. **USGS Earthquakes (Сейсмическая активность):** Глобальный мониторинг сейсмических событий с магнитудой от 4.0.
6. **Submarine Cables (Оптоволоконные магистрали):** Топология трансокеанских подводных магистралей связи.
...
### 1. Установка из готового DMG-пакета (macOS):
Скачайте официальный образ установщика [ARGUS-1.0.0-arm64.dmg](https://github.com/Evro228/argus/releases/download/v1.0.1/ARGUS-1.0.0-arm64.dmg) и перетащите `ARGUS.app` в папку «Программы» (`/Applications`).
* **Контрольная сумма SHA-256:** `1f2d1ad9e76e4c0b300ec55d1c851d8a53cd3ac5ce92fc9728bfc21952596a45`

> **Примечание безопасности macOS (Gatekeeper):**  
> Поскольку ARGUS распространяется через открытый репозиторий GitHub без платной подписки Apple Developer ($99/год), macOS при скачивании через Safari/Chrome автоматически вешает флаг карантина и может вывести предупреждение: *«Приложение повреждено, и его не удается открыть. Переместите в Корзину»*.  
> **Приложение не повреждено.** Чтобы разрешить запуск, выполните одну команду в Терминале:
> ```bash
> xattr -cr /Applications/ARGUS.app
> ```
> *Либо в «Системные настройки» → «Конфиденциальность и безопасность» → «Безопасность» нажмите кнопку **«Все равно открыть»** (Open Anyway).*

### 2. Запуск нативного десктопного приложения из исходников:
```bash
